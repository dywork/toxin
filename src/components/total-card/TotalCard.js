...
class TotalCard {
  ...
  _hasDatesEmpty() {
    return (
      this.arrivalDate === this.placeholder &&
      this.departureDate === this.placeholder
    );
  }
  ...
  _renderResult = () => {
    this.priceView.textContent = `${getNumStringWithSpaces(
      this._getTotalPrice()
    )}₽`;

    this.costPerDaysView.textContent = `${getNumStringWithSpaces(
      this._getCostPerDays()
    )}₽`;
